    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-start justify-between gap-4 bg-[var(--c-subtle)] rounded-xl p-3.5">
          ...
          <button onClick={() => onGotoStep(2)} className="text-xs text-[var(--c-accent)] hover:underline flex-shrink-0">{tr("แก้ไข")}</button>
        </div>
        ...
        <div className="bg-[var(--c-subtle)] rounded-xl p-3.5 space-y-1.5 text-sm">
          ...
          <div className="flex justify-between text-[var(--c-ink-2)]">
            <span>{tr("เงินมัดจำ")}</span>
            <span>฿{pricing.securityDeposit.toLocaleString()}</span>
          </div>
          <div className="flex justify-between text-[var(--c-ink-2)]">
            <span>{tr("ค่าธรรมเนียม (5%)")}</span>
          ...
          <div className="flex justify-between text-xs text-[var(--c-faint)]">
            <span>{tr("วิธีชำระ")}</span>
            <span>{state.paymentMethod === "ESCROW" ? "💳 Escrow" : tr("💵 COD (มัดจำผ่านระบบ)")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="border border-[var(--c-line)] rounded-xl overflow-hidden">
        <div className="bg-[var(--c-canvas)] px-4 py-2.5 flex items-center gap-2">
          ...
          <p className="text-xs font-bold text-[var(--c-ink-1)]">{tr("เงื่อนไขการเช่า (สัญญาเช่าดิจิทัล)")}</p>
        </div>
        <div className={`px-4 py-3 text-xs text-[var(--c-ink-2)] space-y-1.5 leading-relaxed ${
          expanded ? "" : "max-h-40 overflow-hidden"
        }`}>
          <p>{tr("1. ผู้เช่าตกลงเช่าสินค้าตามระยะเวลาที่ระบุข้างต้น")}</p>
          <p>{tr("2. ผู้เช่าต้องดูแลสินค้าเสมือนของตนเอง และคืนในสภาพเดิม")}</p>
          ...
          <p>{tr("8. ค่าธรรมเนียมระบบ 5% จากค่าเช่า ไม่สามารถขอคืนได้เมื่อเจ้าของตอบรับแล้ว")}</p>
          {item.rentalTerms && (
            <>
              <div className="border-t border-[var(--c-line)] pt-1.5 mt-1.5">
                <p className="font-semibold text-[var(--c-ink-1)] mb-1">{tr("เงื่อนไขเพิ่มเติมจากเจ้าของ:")}</p>
        ...
        >
          {expanded ? tr("ย่อลง ▲") : tr("อ่านเพิ่มเติม ▼")}
        </button>
      </div>
      ...
      <label className="flex items-start gap-3 cursor-pointer">
        ...
        />
        <span className="text-sm text-[var(--c-ink-1)]">{tr("ข้าพเจ้าได้อ่านและยอมรับเงื่อนไขการเช่าข้างต้นทุกประการ")}<span className="text-[var(--c-danger)]">*</span>
        </span>
      </label>
    </div>
  );
