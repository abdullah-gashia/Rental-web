...
export default async function AdminFundPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-[#111] flex items-center gap-2">💙 กองทุนงานภัทร</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "อุปกรณ์ในคลัง", value: `${itemCount} ชิ้น`,                                        color: "text-[#111]" },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-2xl border border-[#e5e3de] p-4">
            <p className="text-xs text-[#9a9590] mb-1">{s.label}</p>
      ...
      <div className="bg-white rounded-2xl border border-[#e5e3de] p-5">
        <h2 className="text-sm font-bold text-[#333] mb-4">ที่มาของรายรับ</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div key={k as string}>
              <p className="text-xs text-[#9a9590]">{k}</p>
              <p className="text-base font-bold text-[#111] mt-0.5">
                <MoneyValue amount={v as number} />
              </p>
            </div>
          ))}
        </div>
        <p className="text-[11.5px] text-[#9a9590] mt-4 leading-relaxed">
      ...
      <div className="bg-white rounded-2xl border border-[#e5e3de] p-5">
        <div className="flex items-center justify-between gap-3 mb-4">
          <h2 className="text-sm font-bold text-[#333]">รายการใช้จ่าย ({entries.length})</h2>
          <Link href="/pattara/fund" className="text-xs font-semibold text-[#e8500a] hover:underline">
            เปิดหน้าจัดการของงานภัทร →
          </Link>
        </div>

        {entries.length === 0 ? (
          <p className="py-10 text-center text-sm text-[#aaa]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#faf9f7] border-b border-[#e5e3de]">
                  <th className="text-left px-4 py-3 font-semibold text-[#555]">วันที่</th>
                  <th className="text-left px-4 py-3 font-semibold text-[#555]">รายการ</th>
                  <th className="text-left px-4 py-3 font-semibold text-[#555]">ประเภท</th>
                  <th className="text-left px-4 py-3 font-semibold text-[#555]">บันทึกโดย</th>
                  <th className="text-right px-4 py-3 font-semibold text-[#555]">จำนวน</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#f0ede7]">
                {entries.map((e) => (
                  <tr key={e.id}>
                    <td className="px-4 py-3 whitespace-nowrap text-xs text-[#777]">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {e.receiptUrl && (
                        <>
                          {" · "}
                          <a href={e.receiptUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-[#e8500a] hover:underline">
                            ใบเสร็จ
                          </a>
                        </>
                      )}
                    </td>
                    <td className="px-4 py-3 text-xs text-[#555]">{SOURCE_LABEL[e.source] ?? e.source}</td>
                    <td className="px-4 py-3 text-xs text-[#777]">{e.recordedBy ?? "—"}</td>
