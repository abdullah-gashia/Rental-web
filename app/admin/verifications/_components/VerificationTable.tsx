  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[#dfe7f2] text-left text-[#64748b]">
        ...
        <tbody className="divide-y divide-[#eaf0f8]">
          {requests.map((r) => {
            ...
            return (
              <tr key={r.id} className="hover:bg-[#f1f5fb] transition">
                <td className="py-3 pr-4">
                  <div className="flex items-center gap-2.5">
                    <div className="relative w-8 h-8 rounded-full overflow-hidden bg-[#dfe7f2] flex-shrink-0">
                      {r.user.image ? (
                        <Image src={r.user.image} alt="" fill className="object-cover" />
                      ) : (
                        <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-[#3d4d66]">
                          {r.user.name?.[0]?.toUpperCase() ?? "?"}
                        </span>
                      )}
                    </div>
                    <div>
                      <p className="font-medium text-[#0f1e35] leading-tight">{r.user.name ?? "—"}</p>
                      <p className="text-xs text-[#64748b]">{r.user.email}</p>
                    </div>
                  </div>
                </td>
                <td className="py-3 pr-4 font-mono text-[#1e2d47]">
                  {r.psuIdNumber.slice(0, 3)}{"•".repeat(r.psuIdNumber.length - 3)}
                </td>
                <td className="py-3 pr-4 text-[#3d4d66]">
                  {r.psuIdType === "STUDENT" ? "นักศึกษา" : "บุคลากร"}
                </td>
                <td className="py-3 pr-4 text-[#64748b]">{date}</td>
                <td className="py-3 pr-4">
                ...
                <td className="py-3">
                  <Link
                    ...
                    className="px-3 py-1.5 bg-[#0f1e35] text-white text-xs rounded-lg hover:bg-[#1e2d47] transition"
                  >
                    ตรวจสอบ →
                  </Link>
                </td>
