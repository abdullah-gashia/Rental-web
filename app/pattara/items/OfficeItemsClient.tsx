  ...
  const tr = useLocaleStore((s) => s.tr);
    ...
    for (const file of Array.from(files).slice(0, 8 - next.length)) {
      try {
        ...
        else setMsg({ ok: false, text: json.error ?? tr("อัปโหลดรูปไม่สำเร็จ") });
      } catch {
        setMsg({ ok: false, text: tr("อัปโหลดรูปไม่สำเร็จ") });
      }
    }
  ...
  return (
    <div className="flex flex-col gap-5">
      <header className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-[22px] font-semibold tracking-[-0.02em] text-[var(--psu-navy)]">{tr("คลังอุปกรณ์")}</h1>
          <p className="text-[13px] text-[var(--bw-muted)] mt-1">{tr("หนึ่งแถวคือของหนึ่งชิ้นจริง — ติดตามได้ว่าชิ้นไหนอยู่กับใคร")}</p>
        </div>
        <button onClick={openNew} className="bw-btn bw-btn-primary">{tr("+ เพิ่มอุปกรณ์")}</button>
      </header>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          placeholder={tr("ค้นหาชื่อ / รหัสครุภัณฑ์")}
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-[var(--bw-ground)] border-b border-[var(--bw-line)]">
                ...
                <th className="text-left px-4 py-3 font-semibold text-[var(--bw-ink-2)]">{tr("หมวด")}</th>
                <th className="text-left px-4 py-3 font-semibold text-[var(--bw-ink-2)]">{tr("สถานะ")}</th>
                <th className="text-left px-4 py-3 font-semibold text-[var(--bw-ink-2)]">{tr("อยู่กับใคร")}</th>
                <th className="text-right px-4 py-3 font-semibold text-[var(--bw-ink-2)]">{tr("ให้ยืมแล้ว")}</th>
                <th className="text-right px-4 py-3 font-semibold text-[var(--bw-ink-2)]">{tr("จัดการ")}</th>
              </tr>
            </thead>
            <tbody>
              {shown.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center py-14 text-[var(--bw-muted)]">
                    {items.length === 0 ? tr("ยังไม่มีอุปกรณ์ในคลัง — กด “เพิ่มอุปกรณ์” เพื่อเริ่ม") : tr("ไม่พบรายการที่ตรงกับตัวกรอง")}
                  </td>
                </tr>
              ) : shown.map((i) => (
                <tr key={i.id} className="border-b border-[var(--bw-line)] last:border-0 hover:bg-[var(--bw-tint)] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1.5">
                      <button onClick={() => openEdit(i)} className="bw-btn bw-btn-ghost !h-8 !px-3 !text-[12px]">{tr("แก้ไข")}</button>
                      {i.status === "AVAILABLE" && (
                        <button
        ...
        <div className="fixed inset-0 z-[500] flex items-start justify-center p-4 overflow-y-auto" role="dialog" aria-modal>
          ...
          <div className="relative bg-[var(--c-surface)] rounded-2xl w-full max-w-2xl my-8 p-6 border border-[var(--bw-line)]">
            <h2 className="text-[17px] font-semibold text-[var(--psu-navy)] mb-5">
              {editing.id ? tr("แก้ไขอุปกรณ์") : tr("เพิ่มอุปกรณ์ใหม่")}
            </h2>

            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-1 sm:grid-cols-[1fr_180px] gap-4">
                <div>
                  <label className="bw-label block mb-1.5">{tr("ชื่ออุปกรณ์")}</label>
                  <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="bw-input" placeholder={tr("เช่น เครื่องคิดเลขวิทยาศาสตร์ Casio fx-991")} />
                </div>
                <div>
                  <label className="bw-label block mb-1.5">{tr("รหัสครุภัณฑ์")}</label>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div>
                  <label className="bw-label block mb-1.5">{tr("ยืมได้นานสุด")}</label>
                ...
                <div>
                  <label className="bw-label block mb-1.5">{tr("ยืมขั้นต่ำ")}</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
                <label className="flex items-center gap-2 text-[13px] cursor-pointer">
                  <input type="checkbox" checked={form.isRenewable}
                    ...
                    className="accent-[var(--psu-blue)]" />{tr("ต่ออายุได้")}</label>
                <div>
                  <label className="bw-label block mb-1.5">{tr("ต่ออายุได้กี่ครั้ง")}</label>
                ...
                <div>
                  <label className="bw-label block mb-1.5">{tr("ราคาที่ซื้อมา (บาท)")}</label>
                  <input type="number" min={0} value={form.purchasePrice ?? ""}
                    ...
                    className="bw-input" placeholder={tr("ไม่บังคับ")} />
                </div>
              </div>

              <div>
                <label className="bw-label block mb-1.5">{tr("จุดรับ–คืนของ (คั่นด้วยจุลภาค)")}</label>
                <input
                  value={form.meetupLocations.join(", ")}
                  onChange={(e) => setForm({ ...form, meetupLocations: e.target.value.split(",").map((x) => x.trim()).filter(Boolean) })}
                  className="bw-input"
                  placeholder={tr("สำนักงานงานภัทร ชั้น 1, ห้องกิจการนักศึกษา")}
                />
              </div>

              {!editing.id && purchases.length > 0 && (
                <div>
                  <label className="bw-label block mb-1.5">{tr("ผูกกับใบซื้อในกองทุน")}</label>
                  <select
